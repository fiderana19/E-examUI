...
const GeneratePdf: FunctionComponent<PdfProps> = ({
  audiences,
  total,
  audience_status,
  date_debut,
        ...
        <Document>
          <Page size="A4" style={mininterInfo.page}>
            <View style={mininterInfo.section}>
              ...
              <Text style={mininterInfo.sg}>
                N° <Text style={mininterInfo.liner}> </Text> - MININTER/SG
              </Text>
              <View>
                <Text style={mininterInfo.title}>
                  AUDIENCE {String(audience_status).toUpperCase()} DU{" "}
                  {formatDateForPdf(new Date(date_debut))} AU{" "}
                  {formatDateForPdf(new Date(date_end))}
                </Text>
              </View>
                ...
                {audiences.length > 0 &&
                  audiences.map((audi: any, index: any) => (
                    <View style={audience.row} key={index}>
                      <View style={audience.cell}>
                        <Text style={audience.fsize}>
                          {audi.user_nom} {audi.user_prenom}
                        </Text>
                      </View>
                      <View style={audience.cell}>
                      ...
                      <View style={audience.cell}>
                        <Text style={audience.fsize}>
                          {audi.availability_date} de{" "}
                          {audi.availability_hour_debut} à{" "}
                          {audi.availability_hour_end}{" "}
                        </Text>
                      </View>
                    </View>
                  ))}
              ...
              <Text style={audience.total}>
                Arreté au nombre total de {total} audience(s) .
              </Text>
